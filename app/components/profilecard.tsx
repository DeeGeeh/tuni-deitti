    ...
    return (
      <div className="w-full max-w-sm mx-auto h-screen flex items-center justify-center">
        <div className="text-center p-8 bg-white rounded-xl shadow-lg">
          <h3 className="text-xl font-bold text-gray-800 mb-4">Ei enempää profiileja</h3>
          <p className="text-gray-600">Tule takaisin myöhemmin nähdäksesi lisää frendejä!</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full max-w-sm mx-auto h-screen flex items-center justify-center">
      ...
      >
        ...
        <div className="absolute bottom-6 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4 text-white">
        ...
        <div className="absolute bottom-0 left-0 right-0 flex justify-center space-x-8 mb-4">
          <button 
        ...
        <div
          ...
          style={{ opacity: offsetX < -20 ? Math.min(Math.abs(offsetX) / 100, 1) : 0 }}
        >
          NOPE
        </div>
        <div
        ...
        <div className="text-sm space-y-2 text-black ">
              {currentProfile.questions?.map((question, index) => (
                <p key={index} className="text-sm bg-gray-100 p-2 rouded-md">{question}</p>
              ))}
        </div>
      </div>
    </div>
